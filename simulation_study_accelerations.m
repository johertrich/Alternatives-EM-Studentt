...
%
% This script performs a simulation study as in Table 3 and 4  in the paper and
% saves the result in a text document called 'results_with_accelerations'

clear all
addpath('algorithms')
delete results_with_accelerations;
d=2;
n=1000;
...
for i=1:size(nus,2)
    ...
    for iter=1:niter
        ...
        [~,~,~,num_steps_ecme,time_ecme,~]=iterate_studentT(X,w,'ECME',anz_steps,1);
        
        % Estimating using the GMMF with DAAREM
        [~,~,~,num_steps_daarem_gmmf,time_daarem_gmmf,~]=daarem_studentT(X,w,'GMMF',anz_steps,1);
        
        % Estimating using the MMF with DAAREM
        [~,~,~,num_steps_daarem_mmf,time_daarem_mmf,~]=daarem_studentT(X,w,'MMF',anz_steps,1);
        
        % Estimating using the EM-Algorithm with DAAREM
        [~,~,~,num_steps_daarem_em,time_daarem_em,~]=daarem_studentT(X,w,'EM',anz_steps,1);
        
        % Estimating using the accelerated EM-Algorithm with DAAREM
        [~,~,~,num_steps_daarem_aem,time_daarem_aem,~]=daarem_studentT(X,w,'aEM',anz_steps,1);
        
        % Estimating using the ECME-Algorithm with DAAREM
        [~,~,~,num_steps_daarem_ecme,time_daarem_ecme,~]=daarem_studentT(X,w,'ECME',anz_steps,1);
        
        % Estimating using the GMMF with SQUAREM
        [~,~,~,num_steps_squarem_gmmf,time_squarem_gmmf,~]=squarem_studentT(X,w,'GMMF',anz_steps,1);
        
        % Estimating using the MMF with SQUAREM
        [~,~,~,num_steps_squarem_mmf,time_squarem_mmf,~]=squarem_studentT(X,w,'MMF',anz_steps,1);
        
        % Estimating using the EM algorithm with SQUAREM
        [~,~,~,num_steps_squarem_em,time_squarem_em,~]=squarem_studentT(X,w,'EM',anz_steps,1);
        
        % Estimating using the accelerated EM algorithm with SQUAREM
        [~,~,~,num_steps_squarem_aem,time_squarem_aem,~]=squarem_studentT(X,w,'aEM',anz_steps,1);
        
        % Estimating using the ECME algorithm with SQUAREM
        [~,~,~,num_steps_squarem_ecme,time_squarem_ecme,~]=squarem_studentT(X,w,'ECME',anz_steps,1);
    ...
    fileID=fopen('results_with_accelerations','at');
